...
DEFAULT_INPUT = project_root / "data/bike_events_10x.csv"
...
def run_main(cmd_args, timeout_s: float or None):
    ...
    try:
        ...
        while proc.poll() is None:
            ...
            if p:
                try:
                    # Record process CPU %, RSS memory, and thread count
                    samples.append((p.cpu_percent(interval=None), p.memory_info().rss, p.num_threads()))
                except psutil.NoSuchProcess:
                    break  # Process ended
    ...
    summary = {"cpu_avg_pct": 0.0, "cpu_peak_pct": 0.0, "ram_avg_bytes": 0, "ram_peak_bytes": 0, "thread_avg_count": 0.0, "thread_peak_count": 0}
    if samples:
        cpu_vals = [s[0] for s in samples if s[0] is not None]
        ram_vals = [s[1] for s in samples]
        thread_vals = [s[2] for s in samples]
        if cpu_vals:
        ...
        if thread_vals:
            summary["thread_avg_count"] = sum(thread_vals) / len(thread_vals)
            summary["thread_peak_count"] = max(thread_vals)
...
def main():
    ...
    baseline_args = ["--input", str(DEFAULT_INPUT),
                     "--output", str(baseline_out),
                     "--policy", "none",
                     "--threads", "100",]
    T0, rc, stderr, baseline_summary = run_main(baseline_args, timeout_s=None)
    if rc != 0:
    ...
    logging.info(f"Baseline thread usage (peak): {baseline_summary['thread_peak_count']}")
    ...
    header = [
        ...
        "cpu_avg_pct", "cpu_peak_pct", "ram_avg_bytes", "ram_peak_bytes",
        "thread_avg_count", "thread_peak_count",
        "return_code", "stderr_snippet"
    ]
    ...
    for percent in latency_percentages:
        ...
        logging.info(f"  -> Finished in {Ta:.2f}s. Matches: {matches}, Recall: {recall:.3f}, Peak Threads: {summary['thread_peak_count']}")
        ...
        row = [
            ...
            f"{summary['thread_avg_count']:.2f}",
            summary['thread_peak_count'],
            rc,
            stderr.replace("\n", " ").strip()[:200]
        ]
