...
def build_simple_sampler(pairs: List[Tuple[any, float]], fallback_choices: List, fallback_p: Optional[List[float]] = None):
...
def build_hot_sequence(
    ...
    for _ in range(a_len):
        ...
        rows.append(make_trip_row(
            t, hop_min, current_sid, next_sid, bike_id, catalog,
    ...
    rows.append(make_trip_row(
        t, b_duration, current_sid, end_b, bike_id, catalog,
...
def build_noise_trip(
    ...
    row = make_trip_row(
        base_time, duration, start_sid, end_sid, bike_id, catalog,
...
def main():
    ...
    ap.add_argument("--source-csv", type=str, required=True,
                    help="Path to the REAL CSV used to estimate distributions.")
    ap.add_argument("--n", type=int, required=True, help="Total number of events to generate.")
    ap.add_argument("--x", type=int, required=True, help="Number of hot-path matches to embed (exact).")
    ap.add_argument("--seed", type=int, default=1234, help="Random seed.")
    ap.add_argument("--out", type=str, default="bike_events_synth.csv", help="Output CSV path.")
    ap.add_argument("--base-date", type=str, default="2014-01-31", help="Base date YYYY-MM-DD.")

    # Hot-end control and hot length distribution
    ap.add_argument("--hot-ends", type=str, default="infer",
                    help="Comma-separated hot-end station ids, or 'infer' to choose top-K end stations.")
    ap.add_argument("--hot-ends-topk", type=int, default=3,
                    help="When --hot-ends=infer, select top-K end stations as hot ends.")
    ap.add_argument("--hot-len-dist", type=str, default="1:0.6,2:0.3,3:0.1",
                    help="Distribution for 'a' length (e.g., '1:0.5,2:0.35,3:0.15').")
    ap.add_argument("--cross-midnight-prob", type=float, default=0.4,
                    help="Probability a hot path starts near 23:45 and crosses midnight.")

    # BikeId distribution for NOISE trips
    ap.add_argument("--bikeid-mode", type=str, default="empirical",
                    choices=["sequential", "uniform", "zipf", "empirical"],
                    help="Distribution mode for bikeid among NOISE trips.")
    ap.add_argument("--bikeid-start", type=int, default=30000, help="Start (inclusive) for bikeid range / sequential.")
    ap.add_argument("--bikeid-end", type=int, default=50000, help="End (inclusive) for bikeid range (uniform/zipf).")
    ap.add_argument("--bikeid-zipf-s", type=float, default=1.2, help="Zipf exponent s (>0), larger means heavier head.")

    # Optional: hour control for noise if you want to override empirical
    ap.add_argument("--override-hour", type=str, default="",
                    help="Optional override for hour distribution, e.g. '7:0.1,8:0.2,...'. If empty, use empirical.")
